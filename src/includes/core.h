...
// FORCE_INLINE void updateBannedMoves(const TeamToIntMap&, const std::array<MarkedTConfigSet::const_iterator, rotation_size>& teams_indexes, MovesSet<rotation_size>& banned_moves, VisitedConfigSet& visited_teams_groups);

// template<int rotation_size=2>
// void generateMoves(const TeamToIntMap&, const MarkedTConfigSet&, MovesSet<rotation_size>&, const Move<rotation_size>&);

// template<int rotation_size=2>
// void generateMovesL1(const TeamToIntMap&, const MarkedTConfigSet&, MovesSet<rotation_size>&);

// template<int rotation_size=2>
// void generateMoves(const TeamToIntMap&, const std::array<MarkedTConfigSet::const_iterator, rotation_size>&, MovesSet<rotation_size>&, const ConfigForMoveGen&, MovesSet<rotation_size>&, VisitedConfigSet&);



// template<int rotation_size=2>
// void alt_generateMovesFromEmptySet(ConfigArray& config_set, MovesSet<rotation_size>& empty_moves_set);


// template<int rotation_size=2>
// void alt_GenerateMovesForNTeams(MoveGenTypes::ConfigSetIteratorArray<rotation_size>&, MovesSet<rotation_size>& moves_set, MovesSet<rotation_size>& banned_moves_set);
...
// void alt_generateMovesFromEmptySet(ConfigArray& config_set, MovesSet<rotation_size>& empty_moves_set){
//     MoveGenTypes::ConfigSetIteratorArray<rotation_size> selected_teams;
//     std::fill(selected_teams.begin(), selected_teams.end(), config_set.m_map.begin());
//     MoveGenTypes::ConfigSetIterator end_of_config_set= config_set.m_map.end();
//     MovesSet<rotation_size> banned_moves_set;
//     auto& moves_set=empty_moves_set;
    
//     while(selected_teams[0]!=end_of_config_set){

//         bool completely_same_team_rotation=
//             std::all_of(selected_teams.begin(), selected_teams.end(), 
//                 [&selected_teams](const MoveGenTypes::ConfigSetIterator& x){
//                     return x==selected_teams[0];
//                 });

//         if(!completely_same_team_rotation){
//             alt_GenerateMovesForNTeams(selected_teams, moves_set, banned_moves_set);
//         }

//         int i=1;
//         ++selected_teams[rotation_size-i];
//         while(rotation_size-i!=0 && selected_teams[rotation_size-i]==selected_teams.end()){
//             selected_teams[rotation_size-i]=config_set.m_multiset.begin();
//             i++;
//             ++selected_teams[rotation_size-i];
//         }
//     }
...
// template<int rotation_size>
// void alt_GenerateMovesForNTeams(
//     MoveGenTypes::ConfigSetIteratorArray<rotation_size>& selected_teams, 
//     MovesSet<rotation_size>& moves_set, 
//     MovesSet<rotation_size>& banned_moves_set){
    
//     // update banned moves
//     MoveGenTypes::TeamMapLinearIteratorWTeamRefArray<rotation_size> chosen_ranks;
//     std::transform(selected_teams.begin(), selected_teams.end(), chosen_ranks.begin(), 
//     [](MoveGenTypes::ConfigSetIterator& team){
//         return {team->m_multiset.begin(), &*team};
//     });
...
// template<int rotation_size>
// void generateMoves(
//     const TeamToIntMap& team_to_int_map, 
...
//         generateMovesL1(team_to_int_map, config_set, empty_moves_set);
//     }else{
...
// template<int rotation_size>
// void generateMovesL1(
//     const TeamToIntMap& team_to_int_map, 
//     const MarkedTConfigSet& config_set, 
//     MovesSet<rotation_size>& empty_moves_set){
...
//     std::array<MarkedTConfigSet::const_iterator, rotation_size> indexes;
//     std::fill(indexes.begin(), indexes.end(), config_set.begin());
//     //basically n-dimensional for loop
//     while(indexes[0]!=config_set.end()){
//         bool completely_same_team_rotation=
//             std::all_of(indexes.begin(), indexes.end(), [&indexes](const MarkedTConfigSet::const_iterator& x){return x==indexes[0];});

//         if(!completely_same_team_rotation){
//             generateMoves(team_to_int_map, indexes, empty_moves_set, teams_list_p, banned_moves, visited_teams_groups);
//         }
//         //incrementing similar to count
//         int i=1;
//         ++indexes[rotation_size-i];
//         while(rotation_size-i!=0 && indexes[rotation_size-i]==config_set.end()){
//             indexes[rotation_size-i]=config_set.begin();
//             i++;
//             ++indexes[rotation_size-i];
//         }
//     }
...
//     const TeamToIntMap& team_to_int_map, 
//     const std::array<ConfigForMoveGen::const_iterator, rotation_size>& teams_indexes,
//     MovesSet<rotation_size>& banned_moves, 
//     VisitedConfigSet& visited_teams_groups){
//     // auto range= std::ranges::views::iota;
...
//     }
//     if(mismatch_cnt==1){
//         auto less=[](const MovePieceNBaseRankPair& p1, const MovePieceNBaseRankPair& p2){
//             return p1.first<p2.first;
//         };
//         std::sort(move_arr.begin(), move_arr.end(), less);
//         do{
//             banned_moves.emplace(move_arr);
//         }while(std::next_permutation(move_arr.begin(), move_arr.end(), less));
//     }
// }
...
// void generateMoves(
//     const TeamToIntMap& team_to_int_map, 
//     const std::array<MarkedTConfigSet::const_iterator, rotation_size>& teams_indexes, 
//     MovesSet<rotation_size>& moves_set, 
//     const ConfigForMoveGen& teams_list_p, 
//     MovesSet<rotation_size>& banned_moves, 
//     VisitedConfigSet& visited_teams_groups){
    
//     using ActualTeamSetIterator = decltype((*teams_indexes[0])->begin());
//     using TeamItTeamMovGenPair= std::pair<ActualTeamSetIterator, const TeamSet*>;
    
//     updateBannedMoves(team_to_int_map, teams_indexes, banned_moves, visited_teams_groups);
...
//         [](const ActualTeamSetIterator::iterator& it) {
//             return std::make_pair((*it)->begin(), &*it);
//         });
//     //basically n-dimensional for loop
//     ActualTeamSetIterator end = rank_indexes[0].second->team_set_ptr->end();
//     ActualTeamSetIterator& begin = rank_indexes[0].first;
//     while (begin != end) {
//         bool completely_same_rank_rotation= std::all_of(rank_indexes.begin(), rank_indexes.end(), 
//         [&rank_indexes](const TeamItTeamMovGenPair& x){
//             return *x.first==*rank_indexes[0].first;
//         });
...
//                     return std::make_pair(pair.second->team_set_ptr, static_cast<BaseRank>(*pair.first));
//                 });
